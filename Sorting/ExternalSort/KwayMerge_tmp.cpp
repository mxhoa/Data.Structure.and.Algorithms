...
public:
    SortThread(const std::string output, std::vector<T> data, int blockId)
    ...
    void sortAndWrite()
    {
        std::cout << "SortThread[" << m_block_id << "]->sortAndWrite() :: START\n";
        std::stringstream fileName;
        fileName << m_output_path << "." << m_block_id;
        std::cout << "output = " << m_output_path << std::endl;
        ...
        m_data.clear();
        inFile.close();
        std::cout << "SortThread[" << m_block_id << "]->sortAndWrite() :: END\n";
    }
};
...
KwayMerge<T>::KwayMerge(const std::string &inFile,
                        const std::string &outFile,
                        int memory)
    : m_input_path(inFile), m_output_path(outFile), _compareFunction(NULL), m_memory(memory), m_file_counter(0)
{
    m_file_size = getFileSize();
    m_num_blocks = 0;
    //m_memory_block = m_memory / m_num_threads;
...
void KwayMerge<T>::blockSort()
{
    ...
    while (*input >> line)
    {
        ...
        if (totalBytes > m_memory || currentSize == m_file_size)
        {
            ...
            //		std::cout << "Hello :: Process CHUNK[" << tmpCounter << "]\n";
            //		//std::this_thread::sleep_for(std::chrono::seconds(5));
            //		//SortThread<T>(iFile, blockData, tmpCounter);
            //		SortThread<T> *t = new SortThread<T>(iFile, blockData, tmpCounter);
            //		SortThread<T>::run(t);
            //		std::cout << "End :: CHUNK [" << tmpCounter << "]\n";
            //		return tmpCounter * tmpCounter;
            //	}));
...
int main()
{
    ...
    KwayMerge<std::string> *sorter = new KwayMerge<std::string>(inFile, outFile, ASCENDING, memory * MB_TO_BYTE);
    sorter->Sort();
